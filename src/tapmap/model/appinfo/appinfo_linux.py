...
class LinuxAppInfoBackend:
    ...
    def resolve_identity(self, exe_path: str) -> ApplicationMetadata:
        ...
        Terminal (verification_status set, nothing deferred) when the
        executable isn't owned by any dpkg package.
        """
    ...
    def resolve_verification(
        ...
        """Resolve package integrity and repo provenance for one executable path.

        Only called when resolve_identity() found an owning package.
        """
        real_path = os.path.realpath(exe_path)
        package = linux_package_info.find_owning_package(real_path)

        if package is None:
            # Package may have been removed since resolve_identity() ran.
            return ApplicationMetadata(
                name=identity.name,
