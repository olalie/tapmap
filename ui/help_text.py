...
HELP_CONTENT = [
    ...
    html.Ul([
        ...
        html.Li([
            ...
            html.Ul([
                ...
                html.Li("Copy GeoLite2-City.mmdb and GeoLite2-ASN.mmdb into that folder."),
    ...
    html.P("Yellow is only a visual hint. Zoom in or approach from another direction to separate nearby endpoints."),
    html.P("Location grouping is a separate mechanism. Endpoints with the same rounded coordinates are shown as a single marker."),
    ...
    html.Table(
        ...
        children=[
            ...
            html.Tbody([
                html.Tr([html.Td("U"),   html.Td("Show unmapped endpoints (missing geolocation)"), html.Td("Window")]),
                html.Tr([html.Td("O"),   html.Td("Show open ports (TCP LISTEN and UDP bound)"),   html.Td("Window")]),
    ...
    html.P("The Unmapped endpoints window lists ESTABLISHED TCP connections not shown on the map because geolocation is missing."),
    html.P("By default, only public endpoints are shown. Enable the toggle to include LAN and local endpoints."),
    html.P("If a window is narrow, some fields may be truncated. Hover a cell to see the full value."),

    html.H2("Open ports"),
    html.P("The Open ports window lists local TCP sockets in LISTEN state and UDP sockets bound to local ports."),
    html.P("TCP LISTEN means a local process is waiting for incoming connections. UDP bound means a local process can receive datagrams on that port."),
    ...
    html.P("Short status messages may appear temporarily after commands such as Clear cache, Show cache in terminal, or Recheck databases."),
    ...
    html.Ul([
        html.Li("EST: Unique remote endpoints seen in ESTABLISHED connections, defined as remote IP and port."),
        html.Li("LOC: Endpoints where the remote IP is local, private, or loopback. Not shown on the map."),
        html.Li("NON_GEO: External endpoints without geolocation. Not shown on the map but available via key U."),
        ...
        html.Li("RLOC: Map locations within GEO after coordinate grouping. Multiple IPs and ASNs may map to the same grouped location."),
    ]),
    ...
    html.P("Open the data folder from that window or from About. Copy the files into it and use Recheck GeoIP databases to enable geolocation without restarting."),
    html.P([
    ...
    html.Ul([
        html.Li("MY_LOCATION: 'none' hides the local marker. Use (lon, lat) for fixed coordinates, or 'auto' to detect from public IP."),
        html.Li("POLL_INTERVAL_MS: Snapshot refresh interval in milliseconds."),
        html.Li("COORD_PRECISION: Decimal precision used to group endpoints into one marker. 3 is approximately 100 meters."),
    ...
    html.Ul([
        ...
        html.Li("ASN and ASN organization identify the network operator, not necessarily the service owner."),
    ...
    html.P("If MY_LOCATION is set to 'auto', TapMap may query one or more external services to detect the public IP address. It stops at the first valid result."),
    html.P("To detect OFFLINE status, TapMap performs short connection checks to 1.1.1.1 and 8.8.8.8."),
]
